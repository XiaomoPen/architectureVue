...
name: "mobilize",
  ...
  methods: {
    ...
    initData(page, size) {//当前页，页大小
      this.$axios.get("http://localhost:7777/tpm/find-mobilize", {params: {pageNum: page, size: size}})
        .then((v) => {
    ...
    likeMgr(page, size, search) {//当前页，页大小
      this.$axios.get("http://localhost:7777/find-mobilize2", {
        params: {
          pageNum: page,
          size: size,
