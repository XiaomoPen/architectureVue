...
<template>
  <div v-loading="loading">
    <div>
      <el-row :gutter="10">
        <el-col :span="2">
          <el-button :gutter="0" type="primary" plain @click="$router.push('/clzjh')">新增</el-button>
        </el-col>
        <el-col :span="5">
  ...
  export default {
    ...
    methods: {
      ...
      search(){
        ...
        this.loading=true;
        this.$axios.get("/material/serchMaterial/0/"+this.searchData).then(v => {
          this.loading=false;
          if(v.data.state==200){
      ...
      handleCurrentChange(val) {
        this.$axios.get("/material/getLimitMaterial/0/"+val).then(v => {
          this.loading=false;
          if(v.data.state==200){
            this.tableDate=JSON.parse(v.data.content);
            for(var i=0;i<this.tableDate.length;i++){
    ...
    created() {
      this.$axios.get("/material/getMaterialPlanSum/0").then(v => {
        if(v.data.state==200){
          this.total=parseInt(v.data.content);
        }
      });
      this.$axios.get("/material/getLimitMaterial/0/1").then(v => {
        this.loading=false;
        if(v.data.state==200){
          this.tableDate=JSON.parse(v.data.content);
          for(var i=0;i<this.tableDate.length;i++){
