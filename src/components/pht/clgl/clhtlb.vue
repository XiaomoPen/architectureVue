          ...
          <el-table-column fixed="right" label="操作">
            <template slot-scope="scope">
              <el-button type="text" size="small"
                @click="$router.push({path:'/clht',query:{data:scope.row.contractNumber}})">查看</el-button>
              <el-button type="text" size="small" v-if="scope.row.contractApproval=='未提交'" @click="submit(scope)">发起审批
              </el-button>
              <el-button type="text" size="small" v-if="scope.row.contractApproval=='未审批'" @click="sure(scope)">同意
              </el-button>
              <el-button type="text" size="small" v-if="scope.row.contractApproval=='未审批'" @click="nosure(scope)">不同意
              </el-button>
              <el-button type="text" size="small"
                v-if="scope.row.contractApproval=='未提交'||scope.row.contractApproval=='审批未通过'" @click="deletes(scope)">删除
              </el-button>
            </template>
          </el-table-column>
        </el-table>
        ...
        this.$axios.get("/material/getMaterialContractSearch/" + this.searchData).then(v => {
          ...
          if (v.data.state == 200) {
            ...
            for (var i = 0; i < this.tableDate.length; i++) {
              ...
              switch (n) {
                case 0:
                  this.tableDate[i].contractApproval = "未提交";
                  break;
                case 1:
                  this.tableDate[i].contractApproval = "未审批";
                  break;
                case 2:
                  this.tableDate[i].contractApproval = "审批通过";
                  break;
                case 3:
                  this.tableDate[i].contractApproval = "审批未通过";
                  break;
              }
            }
          } else {
