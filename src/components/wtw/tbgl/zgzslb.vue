        ...
        <el-table :data="ruleForm" border style="width: 100%">
          ...
          <el-table-column prop="zgzsSpjg" label="流程状态">
            <template slot-scope="scope">
              ...
              <span v-if="scope.row.zgzsSpjg==1">未审批</span>
              <span v-if="scope.row.zgzsSpjg==2">审批通过</span>
              <span v-if="scope.row.zgzsSpjg==3">审批不通过</span>
            </template>
          </el-table-column>
          <el-table-column fixed="right" label="操作" width="150">
            <template slot-scope="scope">
              <el-button v-if="scope.row.zgzsSpjg==0" type="text" @click="updates(scope.row.zgzsZsbh)" size="small">
                提交审批</el-button>
              <!-- <el-button type="text" size="small">编辑</el-button> -->
              <el-button type="text" v-if="scope.row.zgzsSpjg==0" @click="deletes(scope.row.zgzsZsbh)" size="small">删除
              </el-button>
              <el-button v-if="scope.row.zgzsSpjg==1" type="text" size="small" @click="updates1(scope.row.zgzsZsbh)">
                通过</el-button>
              <el-button type="text" v-if="scope.row.zgzsSpjg==1" size="small" @click="updates2(scope.row.zgzsZsbh)">
                不通过</el-button>
